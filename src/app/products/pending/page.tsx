...
import { Package, DollarSign, Calendar, Tag, Box, CheckCircle, X, ImageIcon, User, Mail, Phone, MapPin } from 'lucide-react';
...
export default function PendingProducts() {
  ...
  const handleApprove = async (productId: number) => {
    try {
      const response = await fetch(`${BACKEND_API_URL}/products/${productId}/approve`, {
        method: 'POST',
        headers: {
  ...
  const handleReject = async (productId: number) => {
    try {
      const response = await fetch(`${BACKEND_API_URL}/products/${productId}/reject`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedProducts.map((product) => (
                ...
                >
                  ...
                  <td 
                    className="px-6 py-4 whitespace-nowrap"
                    onClick={() =>{openProductDetails(product); console.log(product)}}
                  >
          ...
          {products.length > 0 && (
            <div className="px-6 py-4 border-t border-gray-200">
              ...
              <div className="text-sm text-gray-500 text-center mt-2">
                Showing {Math.min((currentPage - 1) * ROWS_PER_PAGE + 1, products.length)} to {Math.min(currentPage * ROWS_PER_PAGE, products.length)} of {products.length} products
              </div>
            </div>
          )}
        </div>
        ...
        {products.length === 0 && (
          <div className="text-center py-12 bg-white shadow-sm rounded-lg mt-4">
            ...
            <p className="mt-1 text-sm text-gray-500">
              All pending products have been reviewed or there are no pending products at the moment.
            </p>
          </div>
        )}
